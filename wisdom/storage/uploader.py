...
class GitHubUploader:
    def __init__(self):
        self._token = os.environ.get("GITPROVIDER_TOKEN") or os.environ.get(
            "GITHUB_TOKEN", ""
        )
        self._repo = (
            os.environ.get("GITPROVIDER_REPO")
    ...
    def upload(self, data: bytes, filename: str | None = None) -> str | None:
        if not self._token or not self._repo:
            logger.warning(
                "GitHub uploader: GITPROVIDER_TOKEN or GITPROVIDER_REPO not set"
            )
            return None
